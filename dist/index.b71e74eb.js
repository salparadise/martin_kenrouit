...
async function getPrice(token = "bitcoin", currency = "usd") {
    let retries = 5;
    let price = -1;
    while(retries > 0 && price == -1)try {
        const url = BASE_URL + `simple/price?ids=${token}&vs_currencies=${currency}`;
        const res = await (0, _crossFetchDefault.default)(url);
...
},{"fs":"jhUEF","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["iJYvl","h7u1C"], "h7u1C", "parcelRequire28bc")
